...
- Current Rust milestone includes pure parity modules and initial config/palette loading:
  ...
  - initial TUI state machine and terminal IO (`ratatui` on the crossterm backend, keyboard/mouse navigation, filtering, nested palettes, delayed dispatch)
...
6. Port TUI state machine and terminal IO. ✅ runtime migrated to `ratatui` on the crossterm backend; find-pane tree rendering and common edit keys ported; remaining parity gaps include selection-range editing polish and broader dogfood validation.
